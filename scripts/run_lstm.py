...
logger = setup_logger("run_lstm")


def load_prices(cfg):
    """
    Returns full OHLCV DataFrames (not just close) —
    required because the LSTM uses all 5 features.
    """
    ...
    for ticker in cfg["assets"]:
        ...
        prices[ticker] = df   # full OHLCV DataFrame
    return prices
...
def main():
    ...
    logger.info("=" * 50)
    logger.info("STEP 1/5 — Downloading market data")
    ...
    logger.info("=" * 50)
    logger.info("STEP 3/5 — Initialising LSTM")
    forecaster = LSTMForecaster(cfg)

    logger.info("=" * 50)
    logger.info("STEP 4/5 — Training LSTM")
    forecaster.fit(prices_dict)

    logger.info("=" * 50)
    logger.info("STEP 5/5 — Evaluating and forecasting")
